...
import QuestionnaireCard from "@/app/home/questionarios/components/card";
...
export default function QuestionnairesPage() {
  ...
  const handleDeleteForm = async (id: number, title: string) => {
    ...
    try {
      ...
      setQuestionnaires((prevQuestionnaires: Questionary[]) => prevQuestionnaires.filter((questionary) => questionary.id !== id));
    } catch (error) {
      console.error("Ocorreu um erro inesperado ao excluir o questionário", error);
      toast({
        title: "Erro",
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 w-full max-w-screen-xl mx-auto">
            {questionnaires.length === 0 ? (
              <div className="w-full max-w-md absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <p className="text-center font-bold">Nenhum questionário foi encontrado 😔</p>
                <p className="text-center text-gray-400 text-sm">Tente criar um novo...</p>
              </div>
            ) : (
              questionnaires.map((questionary) => (
                <QuestionnaireCard
                  ...
                  onView={() => router.push(`/home/questionarios/responder/${questionary.id}`)}
                />
              ))
            )}
          </div>
